...
{
    enum N_Result result;
    U32           dataLen = 0u;

    for(;;)
    {
        result = isotp_receive(&receiver, 10000u);
        if (result == N_OK)
        {
            debug_out("New data has been received, data len is %d, context:", receiver.DL);
            dataLen = 0;
            while (dataLen < receiver.DL)
            {
                debug_out("%02X ", receiver.Buffer[dataLen]);
                dataLen ++;
            }
            debug_out("\r\n");
        }
...
static U32 port_platformTickUs(void)
{
    U32            tickUs;
    struct timeval tv;

    gettimeofday(&tv,NULL);
    tickUs = tv.tv_sec * 1000000 + tv.tv_usec;
...
void main(void)
{
    ...
    retVal = timer_init(port_platformTickUs, TIMER_COUNT_UP, 1u);
    /* 
